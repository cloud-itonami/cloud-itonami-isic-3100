(ns furnituremfg.governor-contract-test
  ...
  test:
    ...
    FurnitureAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Furniture Plant Operations
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-cutting-saw :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest shipment-units-exceeded-is-held-and-unoverridable
  (testing "a shipment proposal whose units would exceed the batch's own logged unit-count -> HOLD"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
      (is (some #{:shipment-units-exceeded} (-> (store/ledger db) last :basis)))
      (is (empty? (store/shipment-history db))))))

(deftest line-finalize-is-held-and-permanently-blocked
  (testing "a proposal that sets :finalize? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:line-finalize-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
...
(deftest invalid-grade-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:quality-grade :premium-plus-showroom}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-grade} (-> (store/ledger db) last :basis)))
    (is (not= :premium-plus-showroom (:quality-grade (store/batch db "batch-001"))) "fabricated grade never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "VOC fumes near finishing line"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
