(ns furnituremfg.operation-test
  "Smoke tests for the compiled FurnitureOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `furnituremfg.governor-contract-test`; the Store contract in
  `furnituremfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FurnitureOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :severity :moderate :description "VOC finish fumes near finishing line"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
