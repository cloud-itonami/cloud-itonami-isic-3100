(ns furnituremfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `furnituremfg.store` ns docstring for why a second (Datomic-
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-units
  (let [s (seeded)]
    ...
    (is (= 150.0 (:shipped-unit-count (store/batch s "batch-001")))
