(ns furnituremfg.store
  "SSoT for the furniture-manufacturing plant-operations coordination
  ...
  Decision 1, this vertical is self-contained (no external furniture-
  manufacturing capability library, no jurisdiction-scoped Datomic-
  parity requirement driving a second backend); a `langchain.db`-
  backed store can be added later behind the same protocol without
  changing any caller.

  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             quality-grade/unit-count/defect-rate
                             record (cutting/joinery/assembly/
                             finishing output). `:verified?` marks
                             ...
                             `:shipped-unit-count` tracks the batch's
                             own cumulative-shipped ground truth.
    - `equipment`         -- a cutting/sanding/finishing-line unit's
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled cutting/sanding/finishing-line
                             maintenance-window DRAFT against a piece
                             of equipment (`furnituremfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound-furniture-shipment
                             DRAFT (`furnituremfg.registry`'s
                             `register-shipment`).
  ...
  audit trail a factory owner or furniture buyer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (furnituremfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (furnituremfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :quality-grade :grade-a :product "dining-chair"
                :unit-count 500.0 :defect-rate-percent 2.0
                :verified? true :registered? true
                :shipped-unit-count 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :quality-grade :premium :product "oak-dresser"
                :unit-count 80.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-unit-count 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :quality-grade :commercial :product "office-desk"
                :unit-count 600.0 :defect-rate-percent 4.0
                :verified? false :registered? false
                :shipped-unit-count 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :cutting-saw
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :finishing-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `furnituremfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `furnituremfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-unit-count]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:unit-count value) 0.0))))))))
        result)
...
(defn sample-data!
  ...
  unit-count -- HARD hold), one UNVERIFIED/unregistered batch (blocks
  any shipment coordinated against it); one verified+registered
  cutting-saw unit (schedulable for maintenance), one
  UNVERIFIED/unregistered finishing-line unit (blocks any maintenance
  scheduling against it, e.g. VOC-ventilation commissioning still
  pending) -- so the actor + demo + tests run offline. Returns `s`
