(ns furnituremfg.operation
  "FurnitureOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FurnitureAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Furniture Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [furnituremfg.advisor :as advisor]
            [furnituremfg.governor :as governor]
            [furnituremfg.phase :as phase]
...
(defn build
  "Compiles a FurnitureOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `furnituremfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FurnitureAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Furniture Plant Operations Governor -- independent censor.
      (g/add-node :govern
